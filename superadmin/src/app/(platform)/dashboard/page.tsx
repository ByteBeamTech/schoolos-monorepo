"use client";
import { Building2, Users, CreditCard, ShieldAlert, TrendingUp, Activity } from "lucide-react";
import { StatCard }   from "@/components/ui/stat-card";
import { PageHeader } from "@/components/ui/page-header";
import { Badge }      from "@/components/ui/badge";
import { useApi, Tenant, FraudAlert } from "@/lib/hooks";
import { formatCurrency, formatDate, formatRelative } from "@/lib/utils";

function tenantStatusVariant(s: string) {
...
export default function DashboardPage() {
  const { data: tenants, loading: tLoading } = useApi<{ data: Tenant[]; meta: { total: number } }>("/tenants?limit=5");
  const { data: alerts,  loading: aLoading } = useApi<FraudAlert[]>("/fraud/alerts?status=OPEN&limit=5");
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="SchoolOS SaaS control center"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800">
          <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            ...
            <a href="/dashboard/tenants" className="text-xs text-orange-400 hover:text-orange-300">View all →</a>
          </div>
          <div className="divide-y divide-slate-800">
            {tLoading ? (
              ...
              <a key={t.id} href={`/dashboard/tenants/${t.id}`} className="px-5 py-3.5 flex items-center justify-between hover:bg-slate-800/50 transition-colors group">
                <div>
                  <p className="text-sm font-medium text-slate-200 group-hover:text-white">{t.name}</p>
                  <p className="text-xs text-slate-500">{t.slug} · {t.region} · {formatRelative(t.createdAt)}</p>
                </div>
                <Badge label={t.status} variant={tenantStatusVariant(t.status)} />
              </a>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800">
          <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            ...
            <a href="/dashboard/fraud" className="text-xs text-orange-400 hover:text-orange-300">View all →</a>
          </div>
          <div className="divide-y divide-slate-800">
            {aLoading ? (
              [...Array(4)].map((_, i) => (
              ...
              <div key={a.id} className="px-5 py-3.5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-600 mt-0.5">{a.tenant?.name ?? "—"} · {formatRelative(a.createdAt)}</p>
                  </div>
        ...
        <div className="p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className="flex items-center gap-2.5">
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${status === "operational" ? "bg-emerald-500" : "bg-amber-400"}`} />
              <div>
                <p className="text-xs font-medium text-slate-300">{label}</p>
                <p className={`text-[10px] ${status === "operational" ? "text-emerald-500" : "text-amber-400"}`}>
