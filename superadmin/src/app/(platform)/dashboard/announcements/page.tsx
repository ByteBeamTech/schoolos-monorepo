...
export default function AnnouncementsPage() {
  const { data: tenants } = useApi<any>("/tenants?limit=200");
  const tenantList = tenants?.data ?? [];

  const [form, setForm] = useState({
    title: "", body: "", channel: "EMAIL",
    targetAll: true, targetTenantId: "",
    targetStatus: "", targetRegion: "",
  });
  ...
  const send = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const targets = form.targetAll
        ? tenantList.map((t: any) => t.id)
        : form.targetTenantId ? [form.targetTenantId] : [];

      let queued = 0;
      for (const tenantId of targets) {
        await api.post("/notifications/send", {
          channel: form.channel, subject: form.title, body: form.body,
        }).catch(() => null);
        queued++;
      }
        ...
        <form onSubmit={send} className="space-y-4">
          ...
          <div>
            ...
            <label className="flex items-center gap-2 cursor-pointer mb-2">
              <input type="checkbox" checked={form.targetAll} onChange={f("targetAll")} className="accent-orange-500 w-4 h-4" />
            ...
            {!form.targetAll && (
              <select value={form.targetTenantId} onChange={f("targetTenantId")}
                ...
                {tenantList.map((t: any) => <option key={t.id} value={t.id}>{t.name} ({t.slug})</option>)}
              </select>
            )}
          </div>
