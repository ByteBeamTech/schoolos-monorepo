...
export default function HealthPage() {
  const { data, loading } = useApi<any>("/superadmin/health");
  const [filter, setFilter] = useState<"all"|"healthy"|"at_risk"|"critical">("all");

  const summary = data?.summary ?? {};
  const scores  = (data?.scores ?? []).filter((s: any) => filter === "all" || s.tier === filter);

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white tracking-tight">Tenant Health Scores</h1>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {loading ? [...Array(8)].map((_,i) => (
              ...
              <tr><td colSpan={7} className="px-5 py-12 text-center text-slate-500">No tenants found</td></tr>
            ) : scores.map((s: any) => (
              <tr key={s.id} className="hover:bg-slate-800/30">
                ...
                <td className="px-5 py-3.5 text-slate-400">{s.signals.logins7d}</td>
                <td className="px-5 py-3.5 text-slate-400">{s.signals.students}</td>
                <td className="px-5 py-3.5">
                  <span className={`text-xs ${
                    s.signals.subStatus==="ACTIVE"?"text-emerald-400":
                    s.signals.subStatus==="PAST_DUE"?"text-red-400":"text-slate-400"
                  }`}>{s.signals.subStatus}</span>
                </td>
                <td className="px-5 py-3.5 text-xs text-slate-400">
                  {s.daysToExpiry !== null ? (
                    <span className={s.daysToExpiry <= 3 ? "text-red-400 font-medium" : s.daysToExpiry <= 7 ? "text-amber-400" : "text-slate-400"}>
