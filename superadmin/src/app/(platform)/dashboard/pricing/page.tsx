...
import { useApi, PricingPlan } from "@/lib/hooks";
...
function tierVariant(t: string) {
  if (t === "ENTERPRISE") return "purple" as const;
  if (t === "PRO")        return "info" as const;
  if (t === "GROWTH")     return "success" as const;
  return "neutral" as const;
}

export default function PricingPage() {
  const { data, loading, refetch } = useApi<PricingPlan[]>("/saas/pricing-plans");
  const plans = Array.isArray(data) ? data : [];

  const [showModal, setShowModal] = useState(false);
  const [saving, setSaving]       = useState(false);
  const [form, setForm] = useState({
    name: "", tier: "STARTER", model: "SUBSCRIPTION", currency: "INR",
    region: "IN", baseFee: "", perStudentRate: "", studentLimit: "",
    overageRate: "", overageEnabled: false, trialDays: "30",
  });

  const save = async () => {
    setSaving(true);
    try {
      await api.post("/saas/pricing-plans", {
        ...form,
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {plans.map(plan => (
            <div key={plan.id} className={`bg-slate-900 rounded-xl border p-5 ${plan.isActive ? "border-slate-700" : "border-slate-800 opacity-60"}`}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-bold text-white text-base">{plan.name}</h3>
                  <p className="text-xs text-slate-500 mt-0.5">{plan.region} · {plan.currency}</p>
                </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 flex items-center gap-2">
              <input type="checkbox" id="overage" checked={form.overageEnabled} onChange={f("overageEnabled")} className="accent-orange-500" />
              <label htmlFor="overage" className="text-sm text-slate-300">Enable overage billing</label>
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 text-sm bg-slate-800 border border-slate-700 text-slate-300 rounded-lg hover:bg-slate-700 transition-colors">
              Cancel
            </button>
