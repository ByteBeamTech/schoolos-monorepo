...
export default function ImpersonatePage() {
  ...
  const doImpersonate = async () => {
    ...
    if (!confirm(`You will log in as the SCHOOL_ADMIN of the selected school.\n\nReason: ${reason}\n\nThis action is logged. Continue?`)) return;
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Log into any school's dashboard as their admin. Every session is logged.</p>
      </div>
      ...
      <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-amber-500 mt-0.5">Every session is logged in the tenant's audit trail with your identity, reason, and timestamp. The session expires in 30 minutes automatically.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 max-w-lg">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Target school *</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Reason (required, logged in audit) *</label>
          ...
          <button onClick={doImpersonate} disabled={loading || !selectedTenant || !reason.trim()}
        ...
        {result && (
          <div className="mt-5 bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-4">
            ...
            <a href={result.frontendUrl} target="_blank" rel="noreferrer"
