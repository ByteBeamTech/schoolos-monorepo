...
// ── Shared helper to call API and show toast ──────────────────────────────────
...
function ProfileTab() {
  ...
  const save = () => apiCall(
    () => apiClient.patch("/school-management/profile", form),
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="md:col-span-3">
            <Field label="Street Address" value={form.address} onChange={f("address")} />
          </div>
...
function BranchesTab() {
  ...
  const create = async () => {
    const ok = await apiCall(
      () => apiClient.post("/school-management/branches", form),
      "Branch created",
      setSaving,
    );
    if (ok) { setShow(false); setForm({ name:"",code:"",address:"",city:"",phone:"",email:"",principal:"" }); refetch(); }
  };

  const remove = async (id: string) => {
    if (!confirm("Deactivate this branch?")) return;
    await apiCall(
      () => apiClient.delete(`/school-management/branches/${id}`),
      "Branch deactivated",
      setSaving,
    );
    refetch();
      ...
      {loading
        ...
        : (
          <Card className="p-0 overflow-hidden">
            {!data?.length
              ? <p className="p-8 text-center text-slate-400 text-sm">No branches yet.</p>
              : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      {["Name","Code","City","Principal","Phone",""].map(h => (
...
function UsersTab() {
  ...
  const remove = async (id: string) => {
    ...
    await apiCall(
      () => apiClient.delete(`/school-management/users/${id}`),
      "User removed",
      setSaving,
    );
    refetch();
  ...
  const roleColor = (r: string) =>
    ({ SCHOOL_ADMIN:"bg-purple-100 text-purple-700", PRINCIPAL:"bg-blue-100 text-blue-700",
       ACCOUNTANT:"bg-green-100 text-green-700", TEACHER:"bg-amber-100 text-amber-700" }[r]
              ...
              {(data?.classes ?? []).map((c: any) => (
                <div key={c.id} className="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-lg">
                  ...
                  {c.sections?.map((s: any) => (
                    <span key={s.id} className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{s.name}</span>
                  ))}
                </div>
              ))}
              {!data?.classes?.length && <p className="text-sm text-slate-400 py-2">No classes yet.</p>}
            </div>
          )}
      </Card>
        ...
        {!loading && (
          <div className="flex flex-wrap gap-2">
            {(data?.subjects ?? []).map((s: any) => (
              <span key={s.id} className="text-sm bg-slate-100 text-slate-700 px-3 py-1 rounded-full">
                {s.name}{s.code && <span className="text-slate-400 text-xs ml-1">({s.code})</span>}
              </span>
            ))}
            {!data?.subjects?.length && <p className="text-sm text-slate-400">No subjects yet.</p>}
          </div>
        )}
      </Card>
    </div>
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        {showR && (
          <div className="grid grid-cols-3 gap-3 mb-4 pb-4 border-b border-slate-100">
            ...
            <div className="flex items-end gap-2">
              <SaveBtn saving={saving} onClick={() => post(
                "/school-management/transport/routes",
                { ...rf, feeAmount: Number(rf.feeAmount) || undefined },
                () => { setShowR(false); setRf({ name:"",description:"",feeAmount:"" }); }
              )} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {(data?.routes ?? []).map((r: any) => (
            <div key={r.id} className="p-3 bg-slate-50 rounded-lg">
              ...
              {r.feeAmount && <p className="text-xs text-green-700 mt-1">₹{r.feeAmount.toLocaleString("en-IN")}/mo</p>}
            </div>
          ))}
          {!loading && !data?.routes?.length && <p className="text-sm text-slate-400">No routes yet.</p>}
        </div>
      </Card>
      ...
      <Card>
        ...
        {showV && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4 pb-4 border-b border-slate-100">
            ...
            <div className="flex items-end gap-2">
              <SaveBtn saving={saving} onClick={() => post(
                "/school-management/transport/vehicles",
                { ...vf, capacity: Number(vf.capacity) || undefined },
                () => { setShowV(false); setVf({ registrationNumber:"",model:"",capacity:"",driverName:"",driverPhone:"" }); }
              )} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {(data?.vehicles ?? []).map((v: any) => (
              <div key={v.id} className="p-3 bg-slate-50 rounded-lg">
                <p className="font-mono font-medium text-sm">{v.registrationNumber}</p>
                <p className="text-xs text-slate-500 mt-0.5">{v.model ?? ""}{v.capacity ? ` · ${v.capacity} seats` : ""}</p>
                {v.driverName && <p className="text-xs text-slate-400 mt-1">{v.driverName}{v.driverPhone && ` · ${v.driverPhone}`}</p>}
              </div>
            ))}
            {!data?.vehicles?.length && <p className="text-sm text-slate-400">No vehicles yet.</p>}
          </div>
        )}
      </Card>
    </div>
...
function BrandingTab() {
  ...
  const save = () => apiCall(
    () => apiClient.patch("/school-management/branding", form),
    "Branding saved",
    setSaving,
  );
    ...
    <div className="space-y-5">
      <Card>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          {[["Primary Color","primaryColor","#1E40AF"],["Secondary Color","secondaryColor","#DBEAFE"]].map(([label, key, def]) => (
            <div key={key}>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">{label}</label>
              <div className="flex gap-3 items-center">
                <input type="color" value={form[key] ?? def} onChange={e => f(key)(e.target.value)}
                  className="w-12 h-10 rounded cursor-pointer border border-slate-200" />
                <input type="text"  value={form[key] ?? def} onChange={e => f(key)(e.target.value)}
                  className="flex-1 px-3 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-5 p-4 rounded-xl border border-slate-200" style={{ background: form.secondaryColor ?? "#DBEAFE" }}>
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-sm font-bold"
              style={{ background: form.primaryColor ?? "#1E40AF" }}>S</div>
            <div>
              <p className="font-semibold text-slate-900 text-sm">{form.portalTitle ?? "School Portal"}</p>
              <p className="text-xs text-slate-500">{form.tagline ?? "Your school, our mission."}</p>
            </div>
          </div>
        </div>
      </Card>
...
function SecurityTab() {
  ...
  const save = () => apiCall(
    () => apiClient.patch("/school-management/security", form),
    ...
    <div className="space-y-5">
      <Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          {[
            ["Session Timeout (min)", "sessionTimeoutMinutes", 60],
            ["Max Login Attempts",    "maxLoginAttempts",       5],
            ["Password Expiry (days)","passwordExpiryDays",    90],
          ].map(([label, key, def]) => (
            <div key={key as string}>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">{label}</label>
              <input type="number" value={form[key as string] ?? def}
                onChange={e => f(key as string)(Number(e.target.value))}
                className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          ))}
        </div>
              ...
              <li key={id}>
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-sm rounded-lg text-left transition-colors ${
                    tab === id ? "bg-blue-50 text-blue-700 font-medium" : "text-slate-600 hover:bg-slate-100"
                  }`}
                >
