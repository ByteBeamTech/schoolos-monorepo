import { PrismaClient } from '@prisma/client';
import * as bcrypt from 'bcryptjs';

const prisma = new PrismaClient();
const BCRYPT_ROUNDS = 12;

async function hash(password: string) {
  return bcrypt.hash(password, BCRYPT_ROUNDS);
}

async function main() {
  console.log('\n🌱 Seeding SchoolOS...\n');

  // ─────────────────────────────────────────
  // 1. SUPERADMIN (PLATFORM OWNER)
  // ─────────────────────────────────────────

  const saTenant = await prisma.tenant.upsert({
    where: { slug: 'schoolos-platform' },
    update: {},
    create: {
      name: 'SchoolOS Platform',
      slug: 'schoolos-platform',
      contactEmail: 'superadmin@schoolos.com',
      status: 'ACTIVE',
      featureTier: 'ENTERPRISE',
      region: 'IN',
      currency: 'INR',
    },
  });

  const saPassword = await hash('SchoolOS@2024!');

  await prisma.user.upsert({
    where: {
      tenantId_email: {
        tenantId: saTenant.id,
        email: 'superadmin@schoolos.com',
      },
    },
    update: { passwordHash: saPassword },
    create: {
      tenantId: saTenant.id,
      email: 'superadmin@schoolos.com',
      passwordHash: saPassword,
      firstName: 'SchoolOS',
      lastName: 'Admin',
      role: 'SUPER_ADMIN',
      isActive: true,
      isEmailVerified: true,
    },
  });

  console.log('✅ Superadmin ready');

  // ─────────────────────────────────────────
  // 2. DEMO SCHOOL
  // ─────────────────────────────────────────

  const demoTenant = await prisma.tenant.upsert({
    where: { slug: 'demo-school' },
    update: {},
    create: {
      name: 'Demo International School',
      slug: 'demo-school',
      contactEmail: 'admin@demo-school.com',
      status: 'ACTIVE',
      featureTier: 'GROWTH',
      region: 'IN',
      currency: 'INR',
    },
  });

  const demoPassword = await hash('Demo@123!');

  await prisma.user.upsert({
    where: {
      tenantId_email: {
        tenantId: demoTenant.id,
        email: 'admin@demo-school.com',
      },
    },
    update: { passwordHash: demoPassword },
    create: {
      tenantId: demoTenant.id,
      email: 'admin@demo-school.com',
      passwordHash: demoPassword,
      firstName: 'Demo',
      lastName: 'Admin',
      role: 'SCHOOL_ADMIN', // ✅ FIXED (important)
      isActive: true,
      isEmailVerified: true,
    },
  });

  console.log('✅ Demo school admin ready');

  // ─────────────────────────────────────────
  // 3. BRANCH
  // ─────────────────────────────────────────

  await prisma.branch.upsert({
    where: { id: `br_${demoTenant.slug}_main` },
    update: {},
    create: {
      id: `br_${demoTenant.slug}_main`,
      name: 'Lucknow Main Branch',
      tenantId: demoTenant.id,
      isActive: true,
    },
  });

  console.log('✅ Branch ready');

  // ─────────────────────────────────────────
  // 4. ACADEMIC SESSION
  // ─────────────────────────────────────────

  await prisma.academicSession.upsert({
    where: { id: `session_${demoTenant.slug}_2026` },
    update: {},
    create: {
      id: `session_${demoTenant.slug}_2026`,
      name: '2026-27',
      startDate: new Date('2026-04-01'),
      endDate: new Date('2027-03-31'),
      isCurrent: true,
      tenantId: demoTenant.id,
    },
  });

  console.log('✅ Academic session ready');

  // ─────────────────────────────────────────
  // DONE
  // ─────────────────────────────────────────

  console.log('\n🎉 SEED COMPLETE!\n');

  console.log('🔐 LOGIN DETAILS:');
  console.log('Superadmin → superadmin@schoolos.com / SchoolOS@2024!');
  console.log('Demo Admin → admin@demo-school.com / Demo@123!');
}

main()
  .catch((e) => {
    console.error('❌ Seed failed:', e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
