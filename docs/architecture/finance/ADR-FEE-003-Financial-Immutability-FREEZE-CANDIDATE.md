...
- **Payment**: `PENDING → {PROCESSING → {SUCCESS, FAILED}, FAILED}`. `FAILED` is terminal (a new payment attempt is a new record, not a resurrected one). `SUCCESS` is also terminal as far as `Payment.status` itself is concerned as of M6 below — refunds against a `SUCCESS` payment no longer transition its status.

**`REFUNDED`/`PARTIALLY_REFUNDED` removed from `PaymentStatus` (M6 — this IS an ADR revision, and unlike `OVERDUE` above, the enum value itself is removed, not merely stopped-writing):** these were set independently of the `Refund` records that were supposed to justify them — the exact mechanism behind the defect M2 corrected, where fully refunding one payment among several on an invoice zeroed the invoice's paid amount and re-billed money the institution still held. Per IMM-005, retiring a live transition target **MUST** be treated as a revision to this ADR; this one goes further than IMM-005's minimum by removing the enum value entirely, because — unlike `OVERDUE`, where `SaasInvoice` shares the enum and a genuinely live legacy-row population made a soft transition the safer choice — `PaymentStatus` narrowing was verified clean before removal: repo-wide search confirmed no code path outside `refund/refund.service.ts` reads or writes either value, and the migration backfills every historical row (on both `Payment` and the unrelated `SaasPayment`, which also shares this enum) to `SUCCESS` before narrowing the type, so no row can structurally carry the removed values afterward. `REVERSED` is added to the enum in the same migration, unused until M13's tender-bounce work — built ahead of its producer, the same pattern already used for `FeeHead`'s hierarchy (M9) and `PaymentAllocation`'s `STUDENT_ACCOUNT` funding-source branch (M11).

Refund state is now derived, never persisted: `refund/refund-status.util.ts`'s `refundedAmountFor()`/`derivePaymentRefundState()` compute it from a payment's `Refund` records, the same relationship the `SUCCESS → {REFUNDED, PARTIALLY_REFUNDED}` edge used to describe but without persisting the result. **`refundState`** (`'NONE' | 'PARTIAL' | 'FULL'`), exposed on payment objects returned by `PaymentService.getPaymentHistory()` and embedded in `InvoiceService.findById()`'s response, is the canonical field for "how much of this payment survives"; consumers, frontend or backend, **MUST NOT** re-derive it from `status` themselves — the same rule `isOverdue` established for invoices in the entry above.
...
**IMM-007.** A **reversal** offsets a fact that has already occurred, by recording a new, opposing fact — it **MUST NOT** edit or delete the original. Examples already present in the schema and this ADR ratifies as the correct pattern: a `Refund` reverses part or all of a `SUCCESS` `Payment` (the payment record itself is untouched, including its `status`, which stays `SUCCESS` — see M6 in §4 — and the `Refund` is a separate new record; how much of the payment survives is derived from the `Refund` rows, not written back onto the payment); a `LateFee` moving to `WAIVED` or `REVERSED` records the reversal as a status transition with a reason and actor, not a deletion of the fact that the late fee was once `ACTIVE`.
